...
- [x] `ClassRoom` 엔티티 및 `Enrollment` (수강생 매핑) 엔티티 설계
- [x] 교수용: 새 수업 개설 폼 (수업명, 학기, 분반 등) (`/classes/new`)
- [x] 6자리 랜덤 초대 코드 생성 로직 (SecureRandom 및 중복 방지)
- [x] 학생용: 초대 코드 입력 및 수강 신청 기능 (`/classes/join`)
- [x] 교수/학생 대시보드 (내가 참여 중인 수업 목록 동적 카드 뷰) (`/classes/{id}`, `/`)

### 4. 주차별 강의 자료실 (Materials)
- [x] `WeekSection`(주차) 및 `Material`(강의 자료) 엔티티 설계
- [x] 로컬 파일 스토리지 업로드 서비스 구현 (용량 20MB 제한, 화이트리스트 확장자 검증)
- [x] 주차별(1~16주차) 자료 등록 및 학생 파일 다운로드 기능
- [x] 예약 발행(Scheduled Release) 플래그 및 공개 시점 필터링 로직
